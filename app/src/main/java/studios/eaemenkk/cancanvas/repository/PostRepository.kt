...
class PostRepository (context: Context, baseUrl: String, subscriptionUrl: String): BaseApollo(context, baseUrl, subscriptionUrl) {
    ...
    fun createPost(content: FileUpload, description: String?, bidId: String?, callback: (path: String) -> Unit) {
        apolloClient.mutate(CreatePostMutation(content, Input.optional(description), Input.optional(bidId))).enqueue(object: ApolloCall.Callback<CreatePostMutation.Data>() {
            override fun onFailure(e: ApolloException) {
                println("Apollo Error$e")
            }

            override fun onResponse(response: Response<CreatePostMutation.Data>) {
                response.data?.createPost?.let { callback(it) }
            }
